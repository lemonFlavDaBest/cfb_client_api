...
pub async fn get_elo_ratings(api_client: &ApiClient, params: Option<RatingsEloParams<'_>>) -> Result<TeamEloRating, Error> {
    ...
    let json_response: TeamEloRating = response.json().await?;
    Ok(json_response)
}